...
// 通过 enum 获取对应 label
const getEnumLabel = (value: any, column: ColumnProps) => {
  const fieldNames = column.fieldNames ?? { label: "label", value: "value", children: "children" };
  const enums = enumMap.value.get(column.prop!);
  if (!enums) return undefined;
  const findLabel = (options: any[]): string | undefined => {
    for (const option of options) {
      if (option[fieldNames.value] === value) return option[fieldNames.label];
      const childrenKey = fieldNames.children;
      if (childrenKey && option[childrenKey]) {
        const child = findLabel(option[childrenKey]);
        if (child) return child;
      }
    }
  };
  return findLabel(enums);
};
...
const activeFilters = computed(() => {
  ...
  return searchColumns.value
    .map(column => {
      const key = column.search?.key ?? handleProp(column.prop!);
      const rawValue = unref(searchParam.value[key]);
      if (rawValue === undefined || rawValue === null || rawValue === "") return null;
      if (Array.isArray(rawValue) && !rawValue.length) return null;
      const formatValue = (val: any) => {
        if (Array.isArray(val)) {
          return val.map(item => getEnumLabel(item, column) ?? item).join(" / ");
        }
        return getEnumLabel(val, column) ?? val;
      };
      return {
        key,
...
const clearFilter = (key: string) => {
  const defaultValue = searchInitParam.value[key];
