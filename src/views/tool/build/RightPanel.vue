        ...
        <el-form v-show="currentTab === 'field' && showField" size="small" label-width="90px">
          ...
          <el-form-item v-if="activeData['prefix-icon'] !== undefined" label="前图标">
            <el-input v-model="activeData['prefix-icon']" placeholder="请输入前图标名称">
              <template #append>
                <el-button @click="openIconsDialog('prefix-icon')">
          ...
          <el-form-item v-if="activeData['suffix-icon'] !== undefined" label="后图标">
            <el-input v-model="activeData['suffix-icon']" placeholder="请输入后图标名称">
              <template #append>
                <el-button @click="openIconsDialog('suffix-icon')">
                  <el-icon><Pointer /></el-icon>
                  选择
                </el-button>
              </template>
          ...
          <template v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.tag) > -1">
            ...
            <el-form-item label="数据来源">
              <el-radio-group v-model="activeData.dataSource" size="small" @change="handleDataSourceChange">
                <el-radio-button value="static"> 静态数据 </el-radio-button>
                <el-radio-button value="api"> API接口 </el-radio-button>
              </el-radio-group>
            </el-form-item>

            <!-- 静态数据配置 -->
            <template v-if="!activeData.dataSource || activeData.dataSource === 'static'">
            ...
            <!-- API数据源配置 -->
            <template v-if="activeData.dataSource === 'api'">
              <el-form-item label="API地址">
                <el-input v-model="activeData.apiUrl" placeholder="请输入API地址，如：/monitor/job/list">
                  <template #append>
              ...
              <el-form-item label="请求方法">
                <el-radio-group v-model="activeData.apiMethod" size="small">
                  <el-radio-button value="get">GET</el-radio-button>
                  <el-radio-button value="post">POST</el-radio-button>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="数据路径">
                <el-input v-model="activeData.apiDataPath" placeholder="数据路径，如：rows 或 data.rows">
                  <template #append>
                    <el-tooltip content="从API响应中提取数据的路径，如：rows 表示 response.rows" placement="top">
              ...
              <el-form-item label="标签字段">
                <el-input v-model="activeData.apiLabelKey" placeholder="显示文本的字段名，如：jobName" />
              </el-form-item>
              <el-form-item label="值字段">
                <el-input v-model="activeData.apiValueKey" placeholder="选项值的字段名，如：jobId" />
              </el-form-item>
              <el-form-item label="请求参数">
                <el-input
                  v-model="activeData.apiParams"
                  type="textarea"
                  :rows="3"
                  placeholder='JSON格式的请求参数，如：{"pageNum": 1, "pageSize": 10}'
                />
              </el-form-item>
              <el-alert
                v-if="activeData.apiTestResult"
                :title="activeData.apiTestResult"
                :type="activeData.apiTestSuccess ? 'success' : 'error'"
                :closable="false"
          ...
          <template v-if="['el-cascader'].indexOf(activeData.tag) > -1">
            ...
            <div v-if="activeData.dataType === 'static'" style="margin-left: 20px">
              <el-button style="padding-bottom: 0" type="text" @click="addTreeItem"> 添加父级 </el-button>
            </div>
            <el-divider />
          </template>
          ...
          >
            <el-radio-group v-model="activeData.size">
              ...
              <el-radio-button value="mini"> 迷你 </el-radio-button>
            </el-radio-group>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleDataSourceChange(val) {
      if (val === "api") {
        // 切换到API数据源时，初始化API配置
        if (!this.activeData.apiUrl) {
          this.$set(this.activeData, "apiUrl", "/monitor/job/list");
          this.$set(this.activeData, "apiMethod", "get");
          this.$set(this.activeData, "apiDataPath", "rows");
          this.$set(this.activeData, "apiLabelKey", "jobName");
          this.$set(this.activeData, "apiValueKey", "jobId");
          this.$set(this.activeData, "apiParams", JSON.stringify({ pageNum: 1, pageSize: 10 }, null, 2));
        }
      }
    },
    // 测试API连接
    async testApi() {
      ...
      try {
        this.$set(this.activeData, "apiTestResult", "正在测试...");
        this.$set(this.activeData, "apiTestSuccess", false);

        let params = {};
        if (this.activeData.apiParams) {
          try {
            params = JSON.parse(this.activeData.apiParams);
          } catch (e) {
            this.$message.error("请求参数格式错误，请输入有效的JSON格式");
            this.$set(this.activeData, "apiTestResult", "测试失败：请求参数格式错误");
            this.$set(this.activeData, "apiTestSuccess", false);
            return;
          }
        }
        ...
        let response;
        if (this.activeData.apiMethod === "post") {
          response = await http.post(this.activeData.apiUrl, params);
        } else {
          response = await http.get(this.activeData.apiUrl, params);
        }

        // 根据数据路径提取数据
        let data = response.data || response;
        if (this.activeData.apiDataPath) {
          const paths = this.activeData.apiDataPath.split(".");
          paths.forEach(path => {
        ...
        if (Array.isArray(data) && data.length > 0) {
          const labelKey = this.activeData.apiLabelKey || "label";
          const valueKey = this.activeData.apiValueKey || "value";
          const sample = data[0];

          if (sample[labelKey] !== undefined && sample[valueKey] !== undefined) {
            this.$set(
              this.activeData,
              "apiTestResult",
              `测试成功！获取到 ${data.length} 条数据，示例：${sample[labelKey]} (${sample[valueKey]})`
            );
            this.$set(this.activeData, "apiTestSuccess", true);
          } else {
            this.$set(this.activeData, "apiTestResult", `测试失败：数据中未找到字段 "${labelKey}" 或 "${valueKey}"`);
            this.$set(this.activeData, "apiTestSuccess", false);
          }
        } else {
          this.$set(this.activeData, "apiTestResult", "测试失败：API返回的数据不是数组或数据为空");
          this.$set(this.activeData, "apiTestSuccess", false);
        }
      } catch (error) {
        console.error("API测试失败:", error);
        const errorMsg = error.msg || error.message || "请求出错";
        this.$set(this.activeData, "apiTestResult", `测试失败：${errorMsg}`);
        this.$set(this.activeData, "apiTestSuccess", false);
      }
    }
  }
